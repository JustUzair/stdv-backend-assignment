...
const userSchema = new mongoose.Schema({
  ...
  password: {
    ...
    select: false, // will not be visible by default on select query
  },
  passwordConfirm: {
    type: String,
...
userSchema.pre(/^find/, function (next) {
  this.find({ active: { $ne: false } }); // used to filter the inactive / deleted users
  next();
});
...
userSchema.methods.changedPasswordAfter = function (JWTTimestamp) {
  if (this.passwordChangedAt) {
    const changedTimestamp = parseInt(
      this.passwordChangedAt.getTime() / 1000, //get time in seconds, because JWT stores time in milliseconds
      10
    );
